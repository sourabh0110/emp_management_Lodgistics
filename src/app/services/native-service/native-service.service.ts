      ...
      imageData => {
        ...
        return new Promise((resolve, reject) => {
          if (
            ...
            this.filePath.resolveNativePath(imageData).then(
              filePath => {
  ...
  moveFile(fromPath, toPath, fileName, newFileName) {
    return new Promise((resolve, reject) => {
      if (!window.cordova) {
        ...
        this.file.moveFile(fromPath, fileName, newPath, newFileName).then(
          entry => {
            console.log(entry);
  ...
  copyFile(fromPath, toPath, fileName, newFileName) {
    return new Promise((resolve, reject) => {
      if (!window.cordova) {
        ...
        let fs = this.file.dataDirectory;
        let newPath = fs + toPath + "/";
        this.file.copyFile(fromPath, fileName, newPath, newFileName).then(
          entry => {
            resolve(entry);
          },
          err => {
  ...
  writeBlobFile(path, fileName, blob) {
    return new Promise((resolve, reject) => {
      ...
      if (!window.cordova) {
        ...
        let fs = this.file.dataDirectory;
        let newPath = fs + path + "/";
        this.file.writeFile(newPath, fileName, blob, options).then(
          entry => {
            console.log(entry);
  ...
  getBlob(path, filename) {
    return new Promise((resolve, reject) => {
      if (!window.cordova) {
        ...
        this.file.readAsArrayBuffer(newPath, filename).then(
          arraBuffer => {
